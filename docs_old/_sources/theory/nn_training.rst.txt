...
Fig. :ref`fig:architecture`(a). It contains two input variables, namely the energy
...
each one characterised by a different total integrated intensity evaluated from
...
:math:`k`-th cluster that has been assigned to the $m$-th replica. Given that these replicas
...
:eq:`eq:costfunction_NNtraining` by
...
.. math:: :label: eq:costfunction_NNtraining_v2
   ...
   E_{\ell_k},\ln \left( N_{\rm tot}^{(i_{m,k},j_{m,k})}\right) \right) \right]^2}{\sigma^2_k \left( E_{\ell_k}\right) }  \, ,
   \qquad E_{\ell_k} \le E_{{\rm I},k} \, ,
...
.. math:: :label: eq:random_choice_spectra_v6
   ...
   {\boldsymbol I}_{\rm ZLP}^{(\rm NN)} = \left\{ I_{\rm ZLP}^{({\rm NN})(1)} \left(
   E,\ln \left( N_{\rm tot} \right)\right), \ldots,
   I_{\rm ZLP}^{({\rm NN})(N_{\rm rep})} \left(
   E,\ln \left( N_{\rm tot} \right)\right) \right\}
...
This is illustrated by the scheme of Fig. :ref:`EELStoy`, showing a toy simulation of
...
The second model hyperparameter, denoted by :math:`E_{{\rm II},k}` in Fig. :ref:`EELStoy`,
...
proposed in :cite:p:`Ball:2021leu`, hence further reducing the need for human-specific input
